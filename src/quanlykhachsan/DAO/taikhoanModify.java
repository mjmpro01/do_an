...
public class taikhoanModify {
    ...
    public taikhoanDTO login(taikhoanDTO tk) throws SQLException {
        ...
        String sql = "select b.manv,b.matkhau,a.tenpq from PHANQUYEN A JOIN TAIKHOAN B ON A.MAPQ = B.MAPQ where B.MANV like '%" + tk.getManv() + "%' and B.MATKHAU like '%" + tk.getMatkhau() + "%'";
    ...
    public taikhoanDTO checkPQ(String manv ) throws SQLException {
        String sql = "select a.tenpq from PHANQUYEN A JOIN TAIKHOAN B ON A.MAPQ = B.MAPQ where B.MANV like '%" + manv + "%'";
        ResultSet r = c.executeQuery(sql);
        if (r.next()) {
            taikhoanDTO temp;
